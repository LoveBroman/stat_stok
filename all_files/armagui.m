function armagui(action)
	...
	% The command armagui opens a window for  ARMA(p,q)-model visualization.
	%
    % Use the buttons to add poles and zeros, either complex or real valued. 
	% With the mouse button you can move the poles and zeros to any position 
    % inside the unit circle (and also outside if you tick off the box 'Demand stability').
	% Point at a pole or zero in the z-plane and delete it (and its conjugate) with the opposite mouse button. 
    % The corresponding covariance function and spectral density can be studied 
    % when you move poles and zeros. The spectral density can be viewed in linear 
    % or logarithmic power scale (Use 'Log scale' for better view of the influence of the zeros). 
    % The button 'Simulate' gives a new realization 
    % of the process.
	
	% Predefined A- and C-polynomial can be loaded into armagui  by creating, e.g.,
	% >> modell.A=[1 -0.5]; 
	% >> modell.C=[1 0.4 0.7];
	% and write the name (i.e.  'model') in the 'Import'-box and push the button.
	% Covariance function, spectral density and realizations can also be
